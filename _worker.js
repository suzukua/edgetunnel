			...
			if (!upgradeHeader || upgradeHeader !== 'websocket') {
				...
				if (路径 == '/') {
					...
					if (env.CFEMAIL && env.CFKEY){
						...
						const accountId = await 获取账号信息(email, key);
						if (accountId){
							const now = new Date()
							now.setUTCHours(0, 0, 0, 0)
							const startDate = now.toISOString()
							const endDate = new Date().toISOString();
							const Sum = await 查询使用量(accountId, accountIndex, email, key, startDate, endDate);
							pagesSum = Sum[0];
							workersSum = Sum[1];
							total = 102400 ;
						}
...
async function 获取账号信息(email, key) {
	try {
...
async function 查询使用量(accountId, accountIndex, email, key, startDate, endDate) {
	try {
